...
export function emit(hookEventName, context) {
  ...
  // Claude Code sets CLAUDE_PLUGIN_ROOT; Codex sets that and PLUGIN_ROOT. Both
  // consume the hookSpecificOutput envelope.
  if ((process.env.CLAUDE_PLUGIN_ROOT || process.env.PLUGIN_ROOT) && !process.env.COPILOT_CLI) {
