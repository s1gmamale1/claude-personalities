...
export function status(sessionId, root) {
  ...
  let resolved_by;
  if (!s && !sessionId) {
    // No session id at all (Codex). A fresh pending lock is the user's most
    // recent intent — "be Raph" then "who am I?" must not answer "nobody" just
    // because no hook has adopted it yet. Otherwise fall back to the newest
    // real lock. Either way, say how the answer was chosen.
    const pending = readState(PENDING_KEY);
    if (isFreshPending(pending)) {
      s = pending; resolved_by = 'pending';
    } else {
