...
// may not reference the session or the personality lock.
//
// These must match the LOCK, not the word "locked". Locking is ordinary backend
// vocabulary — mutexes, row locks, file locks, locked accounts — and that is
// Donatello's actual domain, so a broad /\bis locked\b/ rule would block exactly
// the specialties this feature exists to enable.
export const SESSION_SCOPED_PATTERNS = [
  /\bthis session\b/i,
  /\bmid-session\b/i,
  /\bnew session\b/i,
  /\bpersonality lock\b/i,
  /\byou are locked\b/i,
  /\blocked (in|into|to) (this|a|the) (session|personality|character|role)\b/i,
];
